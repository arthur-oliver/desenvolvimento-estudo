# Definir funções para cada requisito para testá-los separadamente!
...
# TESTE (OPCIONAL)
# Verificar todos os tel.
#for f in fones:
#    print(f)
# Printar quant. tel
#print(len(fones))

# Gerando lista de números repetidos (texto/str)
#for k in range(10):
#    print(str(k) + str(k)) # "+" junta os textos


# Função 1
def repetido (tel):
    for k in range(10):
        if str(k) + str(k) in tel:
            return ('Nº Chato!')
    return ('Nº Legal!')
# Função 2
def par (tel):
    soma = sum(int(digito) for digito in tel)
    if soma % 2 == 0:
        return ('Nº Legal!')
    return ('Nº Chato!')
# Função 3
...
# Verificação se atende as 3 funções
def valido(tel):
    if repetido(tel) == 'Nº Chato!':
        return 'Nº Chato!'
    elif par(tel) == 'Nº Chato!':
        return 'Nº Chato!'
    elif igual(tel) == 'Nº Chato!':
        return 'Nº Chato!'
    else:
        return 'Nº Legal!'

# for tel in fones:
#     print(f'Tel: {tel} = {valido(tel)}')
resultado = [tel for tel in fones if valido(tel) == 'Nº Legal!']
print(f'Quantidade de números válidos ("Legais") = {len(resultado)}')
...
# c=0
# for tel in fones:
#     if repetido(tel) != 'Nº Chato!' and par(tel) != 'Nº Chato!' and igual(tel) != 'Nº Chato!':
#         c=c+1
# print(c)
