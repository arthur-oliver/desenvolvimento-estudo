...
def not_bad(s):
    for letra in range(len(s) - 2):
        if s[letra:letra+3] == 'not':
            for k in range(letra+3, len(s) - 2):
                if s[k:k+3] == 'bad':
                    return s[:letra] + 'good' + s[k+3:]
    return s
...
def conta2(n):
  ...
  for numero in range (n):
     numero_extenso = numero_extenso + str(numero)
  return numero_extenso.count('2')
...
def inip2(n):
  resultado = ''
  x = 0
  while resultado[:len(str(n))] != str(n):
    resultado = str(2**x)
    x = x + 1
  return x - 1
