...
def double_char(s):
  return ''.join(letra + letra for letra in s)
...
def count_code(s):
  x=0
  for letra in range(len(s)-3):
    if s[letra:letra+2] == 'co' and s[letra+3] == 'e':
      x += 1
  return x
...
def soma_na_lista(n, lista):
  return True if n in [a + b for a in lista for b in lista if a != b] else False
...
def fila_tijolos(n_peq, n_gra, meta):
  return True if (n_peq*1 + n_gra*5 >= meta and n_peq >= meta % 5) else False
